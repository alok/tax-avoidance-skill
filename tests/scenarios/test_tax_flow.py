...
class TaxFlowTest(unittest.TestCase):
    ...
    def test_student_loan_interest_requires_confirmation(self) -> None:
        normalized, artifacts = self.run_case("student_loan_interest_requires_confirmation")
        self.assertEqual(normalized["status"], "ok")
        self.assertEqual(normalized["facts"]["reported_student_loan_interest"]["value"], 1850)
        self.assertEqual(normalized["facts"]["student_loan_interest_deduction"]["value"], 0.0)
        self.assertIn("Confirm the deductible amount explicitly before applying it", artifacts["missing-items.md"])
        self.assertIn("1098-E reported interest", artifacts["tax-dossier.md"])
        self.assertIn("Not applied automatically; confirm the deductible amount before using it.", artifacts["tax-dossier.md"])
        self.assertIn("| Form 1040 | 10 | Adjustments to income | TBD |", artifacts["federal-lines.md"])

    def test_student_loan_interest_explicit_answer_applies(self) -> None:
        normalized, artifacts = self.run_case("student_loan_interest_confirmed")
        self.assertEqual(normalized["status"], "ok")
        self.assertEqual(normalized["facts"]["reported_student_loan_interest"]["value"], 1850)
        self.assertEqual(normalized["facts"]["student_loan_interest_deduction"]["value"], 1200)
        self.assertNotIn("Confirm the deductible amount explicitly before applying it", artifacts["missing-items.md"])
        self.assertIn("Applied deduction: $1,200.00.", artifacts["tax-dossier.md"])
        self.assertIn("| Form 1040 | 10 | Adjustments to income | $1,200.00 |", artifacts["federal-lines.md"])
