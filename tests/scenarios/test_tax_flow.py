...
class TaxFlowTest(unittest.TestCase):
    ...
    def test_supported_but_incomplete_cases(self) -> None:
        for name in (
            ...
            "social_security_missing_taxable_portion",
        ):
            with self.subTest(name=name):
                normalized, artifacts = self.run_case(name)
                self.assertEqual(normalized["status"], "ok")
    ...
    def test_social_security_taxable_portion(self) -> None:
        normalized, artifacts = self.run_case("social_security_taxable_portion_provided")
        self.assertEqual(normalized["status"], "ok")
        self.assertIn("$18,600.00", artifacts["federal-lines.md"])
        self.assertIn("$7,900.00", artifacts["federal-lines.md"])
        self.assertIn("Form 1040 | 6b | Taxable Social Security benefits", artifacts["federal-lines.md"])
