...
class TaxFlowTest(unittest.TestCase):
    ...
    def test_ira_review_5498(self) -> None:
        ...
        self.assertEqual(normalized["facts"]["ira_contributions_reported"]["value"], 4500)
        self.assertEqual(normalized["facts"]["ira_contribution_deduction"]["value"], 0)
        self.assertIn("IRA Review", artifacts["tax-dossier.md"])
        self.assertIn("$4,500.00", artifacts["tax-dossier.md"])
        self.assertIn(
            "Review the documented IRA contributions from Form 5498",
            artifacts["missing-items.md"],
        )
