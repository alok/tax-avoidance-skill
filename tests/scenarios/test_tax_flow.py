...
class TaxFlowTest(unittest.TestCase):
    ...
    def test_itemized_deduction_review(self) -> None:
        ...
        self.assertEqual(normalized["status"], "ok")
        self.assertEqual(normalized["facts"]["mortgage_interest"]["value"], 12850)
        self.assertEqual(normalized["facts"]["charitable_cash"]["value"], 2400)
        self.assertEqual(normalized["facts"]["student_loan_interest_deduction"]["value"], 625)
        self.assertEqual(normalized["deduction_summary"]["itemized_candidate_total"], 15250)
        self.assertIn("Adjustment And Deduction Review", artifacts["tax-dossier.md"])
        self.assertIn("$15,250.00", artifacts["tax-dossier.md"])
        self.assertIn("Student loan interest deduction", artifacts["tax-dossier.md"])
        self.assertIn("Current itemized-deduction candidates total $15,250.00", artifacts["missing-items.md"])
