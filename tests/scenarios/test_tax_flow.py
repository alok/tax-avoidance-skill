...
class TaxFlowTest(unittest.TestCase):
    ...
    def test_supported_but_incomplete_cases(self) -> None:
        for name in (
            ...
            "social_security_needs_taxable_confirmation",
    ...
    def test_social_security_requires_taxable_confirmation(self) -> None:
        normalized, artifacts = self.run_case("social_security_needs_taxable_confirmation")
        self.assertEqual(normalized["status"], "ok")
        self.assertIn(
            "Confirm the taxable portion of Social Security benefits",
