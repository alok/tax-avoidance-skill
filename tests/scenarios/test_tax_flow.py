...
class TaxFlowTest(unittest.TestCase):
    ...
    def test_substantiation_follow_up(self) -> None:
        normalized, artifacts = self.run_case("substantiation_follow_up")
        self.assertEqual(normalized["status"], "ok")
        missing = artifacts["missing-items.md"]
        self.assertIn("Form 5498", missing)
        self.assertIn("Form 1098-E", missing)
        self.assertIn("Form 1098-T", missing)
        self.assertIn("estimated-tax payment confirmations", missing)
