...
class TaxFlowTest(unittest.TestCase):
    ...
    def test_deduction_support_scaffolding(self) -> None:
        normalized, artifacts = self.run_case("deduction_support_household")
        self.assertEqual(normalized["status"], "ok")
        self.assertEqual(normalized["deduction_summary"]["student_loan_interest"], 1800)
        self.assertEqual(normalized["deduction_summary"]["observed_itemized_support"], 10200)
        self.assertIn("## Deduction Support", artifacts["tax-dossier.md"])
        self.assertIn("$10,200.00", artifacts["tax-dossier.md"])
        self.assertIn("$1,800.00", artifacts["tax-dossier.md"])
        self.assertIn("Observed itemized-deduction support currently totals $10,200.00", artifacts["missing-items.md"])
