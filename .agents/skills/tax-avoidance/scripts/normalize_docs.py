...
def normalize_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    ira_contributions_documented, ira_contribution_sources = aggregate_numeric(
        documents,
        {"5498"},
        "ira_contributions",
    )
    ...
    if ira_contributions_documented > 0.0 and "ira_contribution_deduction" not in answers:
        missing_items.append(
            f"Review the documented IRA contributions totaling ${ira_contributions_documented:,.2f} from Form 5498 and confirm how much, if any, should be treated as a deductible IRA contribution."
        )
