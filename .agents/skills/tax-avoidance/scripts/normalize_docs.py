...
def normalize_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    traditional_ira_contributions, traditional_ira_contributions_sources = aggregate_numeric(
        documents,
        {"5498"},
        "traditional_ira_contributions",
    )
    roth_ira_contributions, roth_ira_contributions_sources = aggregate_numeric(
        documents,
        {"5498"},
    ...
    if traditional_ira_contributions > 0.0 and "ira_contribution_deduction" not in answers:
        missing_items.append(
            f"Review Form 5498 traditional IRA contributions totaling ${traditional_ira_contributions:,.2f} and confirm how much is deductible for 2025 before applying any IRA deduction."
        )
    if roth_ira_contributions > 0.0:
        missing_items.append(
            "Form 5498 shows Roth IRA contributions. Keep them in the intake record, but do not treat them as a deductible IRA adjustment."
        )
    ...
    facts = {
        ...
        "traditional_ira_contributions_reported": build_fact(
            ...
            traditional_ira_contributions,
            traditional_ira_contributions_sources,
        ),
