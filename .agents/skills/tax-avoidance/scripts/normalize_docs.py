...
def normalize_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if has_5498 and "ira_contribution_deduction" not in answers:
        if ira_contributions_reported > 0.0:
            missing_items.append(
                "Review the documented IRA contributions from Form 5498 and confirm the deductible amount before applying any IRA deduction."
            )
        else:
            missing_items.append(
                "Extract the Form 5498 IRA contribution amount or upload a clearer copy before reviewing any IRA deduction."
            )
    for document in documents:
