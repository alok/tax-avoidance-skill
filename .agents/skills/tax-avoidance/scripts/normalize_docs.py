...
def normalize_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if has_5498 and "ira_contribution_deduction" not in answers:
        if ira_contributions_reported > 0.0:
            missing_items.append(
                f"Review the Form 5498 IRA contributions totaling ${ira_contributions_reported:,.2f} and confirm how much is deductible before applying any IRA deduction."
            )
        else:
            missing_items.append(
                "Extract the deductible IRA contribution amount from the Form 5498 documents or upload a clearer copy before applying any IRA deduction."
            )
