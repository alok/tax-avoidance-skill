...
def normalize_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if candidate_ira_contributions == 0.0:
        candidate_ira_contributions, candidate_ira_sources = aggregate_numeric(
            documents,
            {"5498"},
    ...
    if candidate_ira_contributions > 0.0 and "ira_contribution_deduction" not in answers:
        missing_items.append(
            f"Review the Form 5498 traditional IRA contributions totaling ${candidate_ira_contributions:,.2f} and confirm how much is deductible before using it on the federal return."
        )
