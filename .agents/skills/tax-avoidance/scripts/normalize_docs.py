...
def normalize_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if ira_contributions_reported > 0.0 and "ira_contribution_deduction" not in answers:
        missing_items.append(
            "A Form 5498 reports IRA contributions totaling "
            f"${ira_contributions_reported:,.2f}. Confirm the deductible IRA amount after applying coverage and income-limit rules."
        )
