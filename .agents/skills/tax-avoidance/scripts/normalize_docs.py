...
def normalize_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if deduction_amount == 0.0 and "deduction_amount" not in answers:
        if candidate_itemized_deductions > 0.0:
            itemized_parts: list[str] = []
            if mortgage_interest > 0.0:
                itemized_parts.append(f"mortgage interest ${mortgage_interest:,.2f}")
            if charitable_cash > 0.0:
                itemized_parts.append(f"charitable cash ${charitable_cash:,.2f}")
            breakdown = ", ".join(itemized_parts)
            missing_items.append(
                "Choose the deduction path and provide the deduction amount to use in the draft package. "
                f"Current candidate itemized deductions total ${candidate_itemized_deductions:,.2f} "
                f"({breakdown}); compare them against the standard deduction and any missing SALT or medical items."
            )
        else:
