...
def normalize_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    ira_contributions, ira_contribution_doc_sources = aggregate_numeric(
        documents,
        {"5498"},
        "ira_contributions",
    )
    ...
    if has_5498 and "ira_contribution_deduction" not in answers:
        if ira_contributions > 0.0:
            missing_items.append(
                "Review the Form 5498 IRA contribution amount and confirm any deductible IRA contribution before applying it as an adjustment to income."
            )
        else:
            missing_items.append(
                "Extract the IRA contribution amount from Form 5498 or upload a clearer copy before reviewing any IRA deduction."
            )
    for document in documents:
