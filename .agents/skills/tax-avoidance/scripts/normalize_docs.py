...
def normalize_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if has_5498 and "ira_contribution_deduction" not in answers:
        if ira_contributions_reported > 0.0:
            missing_items.append(
                "Review the reported IRA contributions from Form 5498 and confirm how much is deductible before applying any IRA deduction."
            )
        else:
            missing_items.append(
                "Extract the IRA contribution amount from Form 5498 or upload a clearer copy before applying any IRA deduction."
            )
