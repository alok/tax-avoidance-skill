...
def normalize_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if deduction_amount == 0.0 and "deduction_amount" not in answers:
        known_deduction_inputs: list[str] = []
        if mortgage_interest > 0.0:
            known_deduction_inputs.append(f"mortgage interest ${mortgage_interest:,.2f}")
        if charitable_cash > 0.0:
            known_deduction_inputs.append(f"charitable cash donations ${charitable_cash:,.2f}")
        if student_loan_interest > 0.0:
            known_deduction_inputs.append(f"student loan interest ${student_loan_interest:,.2f}")

        if known_deduction_inputs:
            missing_items.append(
                "Choose the deduction path and provide the deduction amount to use in the draft package. "
                + "Known deduction inputs so far: "
                + ", ".join(known_deduction_inputs)
                + "."
            )
        else:
