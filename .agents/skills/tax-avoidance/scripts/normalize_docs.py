    ...
    if deduction_amount == 0.0 and "deduction_amount" not in answers:
        if mortgage_interest > 0.0 or charitable_cash > 0.0:
            missing_items.append(
                "Review whether to itemize or take the standard deduction. Mortgage interest and charitable-giving support documents were found, so confirm the deduction amount to use in the draft package."
            )
        else:
