...
def normalize_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if ira_contributions_reported > 0.0 and "ira_contribution_deduction" not in answers:
        missing_items.append(
            f"Review the reported IRA contributions totaling ${ira_contributions_reported:,.2f} from Form 5498 and confirm how much is deductible before applying any IRA deduction."
        )
    for document in documents:
