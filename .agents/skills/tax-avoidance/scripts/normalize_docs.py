...
def normalize_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if traditional_ira_contributions > 0.0 and "ira_contribution_deduction" not in answers:
        missing_items.append(
            f"Review the Form 5498 traditional IRA contributions totaling ${traditional_ira_contributions:,.2f} and confirm how much is deductible before applying an IRA deduction."
        )
    for document in documents:
