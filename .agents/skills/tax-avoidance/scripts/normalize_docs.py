...
def normalize_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    ira_contributions_reported += traditional_ira_contributions
    ira_contributions_reported_sources += traditional_ira_contributions_sources
    ...
    if has_5498 and "ira_contribution_deduction" not in answers:
        if ira_contributions_reported > 0.0:
            missing_items.append(
                "Review the 5498 IRA contribution amounts and confirm how much is deductible before applying any IRA deduction."
            )
        else:
            missing_items.append(
