...
def normalize_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    ira_rollover_contributions, ira_rollover_sources = aggregate_numeric(
        documents,
        {"5498"},
    ...
    if has_5498 and "ira_contribution_deduction" not in answers:
        if ira_contributions_reported > 0.0 or ira_rollover_contributions > 0.0:
            missing_items.append(
                "Review the Form 5498 IRA contribution and rollover amounts and confirm the deductible traditional IRA contribution, nondeductible treatment, or Roth-only treatment before applying an IRA deduction."
            )
        else:
            missing_items.append(
