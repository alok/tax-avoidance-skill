...
def build_dossier(normalized: dict[str, Any], line_items: list[dict[str, Any]]) -> str:
    ...
    ira_rollover_contributions = fact_value(normalized, "ira_rollover_contributions")
    ...
    ira_review_lines = [
        ...
        f"- IRA rollover contributions spotted from 5498 documents: {money(ira_rollover_contributions) if ira_rollover_contributions else '$0.00'}",
        (
            f"- Draft IRA deduction currently applied on Form 1040 line 10: {money(ira_deduction)}"
            if ira_deduction
            else "- No IRA deduction is applied yet. Review deductible, Roth, rollover, and nondeductible treatment before adding one."
        ),
    ]
