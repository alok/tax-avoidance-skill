...
def build_dossier(normalized: dict[str, Any], line_items: list[dict[str, Any]]) -> str:
    ...
    ira_review_lines = [
        (
            f"- Reported traditional IRA contributions from Form 5498 documents: {money(ira_contributions_reported)}"
            if ira_contributions_reported
            else "- No traditional IRA contribution amount was extracted from Form 5498 documents."
        ),
        (
            f"- Draft IRA deduction currently applied on Form 1040 line 10: {money(ira_deduction)}"
            if ira_deduction
            else "- No IRA deduction is applied yet. Review the Form 5498 contribution amount and deductibility limits before adding one."
        ),
    ]
