...
def build_dossier(normalized: dict[str, Any], line_items: list[dict[str, Any]]) -> str:
    ...
    sections = [
        ...
        *connector_lines,
        ...
        f"- Confirmed deductible IRA amount used in the draft: {money(ira_contribution_deduction) if ira_contribution_deduction else '$0.00'}",
        "- Form 5498 can show contribution activity, but deductibility still needs an explicit review before the amount is applied to the return draft.",
        "",
        "## State Follow-Up",
        "",
