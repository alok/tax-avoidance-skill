...
def build_line_items(normalized: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    total_income = wages + interest + dividends + capital_gains + (taxable_social_security if has_taxable_social_security else 0.0) + (net_profit or 0.0)
...
def build_dossier(normalized: dict[str, Any], line_items: list[dict[str, Any]]) -> str:
    ...
    social_security_review_lines = [
        ...
        (
            ...
            if fact_sources(normalized, "taxable_social_security_benefits")
            else "- Taxable Social Security is not applied yet. Review the SSA-1099 worksheet inputs before adding line 6b."
        ),
    ]
