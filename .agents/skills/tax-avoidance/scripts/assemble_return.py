...
def build_dossier(normalized: dict[str, Any], line_items: list[dict[str, Any]]) -> str:
    ...
    ira_review_lines = [
        f"- Reported IRA contributions spotted from Form 5498 documents: {money(ira_contributions_reported) if ira_contributions_reported else '$0.00'}",
        (
            f"- Draft IRA deduction currently applied on Form 1040 line 10: {money(ira_deduction)}"
            if ira_deduction
            else "- No IRA deduction is applied yet. Confirm the deductible IRA amount before adding one."
        ),
    ]
