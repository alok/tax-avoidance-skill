...
def build_dossier(normalized: dict[str, Any], line_items: list[dict[str, Any]]) -> str:
    ...
    sections = [
        ...
        *connector_lines,
        ...
        "## Payments Summary",
        "",
        f"- W-2 withholding captured: {money(fact_value(normalized, 'federal_withholding'))}",
        f"- Estimated tax payments captured: {money(fact_value(normalized, 'estimated_tax_payments'))}",
        f"- Other draft payments captured: {money(fact_value(normalized, 'other_payments'))}",
        f"- Total draft payments: {money(next((item['value'] for item in line_items if item['form'] == 'Form 1040' and item['line'] == '33'), None))}",
        "",
