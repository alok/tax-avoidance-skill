...
def build_line_items(normalized: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            "value": taxable_social_security if fact_sources(normalized, "taxable_social_security") else None,
...
def build_dossier(normalized: dict[str, Any], line_items: list[dict[str, Any]]) -> str:
    ...
    taxable_social_security = fact_value(normalized, "taxable_social_security")
    social_security_lines = [
        f"- Gross Social Security benefits spotted from SSA-1099 documents: {money(social_security_benefits) if social_security_benefits else '$0.00'}",
        (
            f"- Taxable Social Security currently applied on Form 1040 line 6b: {money(taxable_social_security)}"
            if fact_sources(normalized, "taxable_social_security")
            else "- No taxable Social Security amount is applied yet. Review the SSA-1099 and confirm the taxable portion before adding one."
        ),
    ]
