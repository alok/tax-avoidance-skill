...
def build_dossier(normalized: dict[str, Any], line_items: list[dict[str, Any]]) -> str:
    ...
    social_security = fact_value(normalized, "social_security_benefits")
    social_security_taxable = fact_value(normalized, "social_security_taxable_benefits")
    benefits_review_lines = [
        f"- Gross Social Security benefits spotted from SSA-1099 documents: {money(social_security) if social_security else '$0.00'}",
        (
            f"- Taxable Social Security currently applied on Form 1040 line 6b: {money(social_security_taxable)}"
            if social_security_taxable
            else "- No taxable Social Security amount is applied yet. Review the SSA-1099 details before adding one to Form 1040 line 6b."
        ),
    ]
