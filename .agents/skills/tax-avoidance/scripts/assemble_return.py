...
def build_dossier(normalized: dict[str, Any], line_items: list[dict[str, Any]]) -> str:
    ...
    retirement_review_lines = [
        f"- IRA contributions spotted from Form 5498 documents: {money(ira_contributions_reported) if ira_contributions_reported else '$0.00'}",
        f"- IRA rollover contributions spotted from Form 5498 documents: {money(ira_rollover_contributions) if ira_rollover_contributions else '$0.00'}",
        (
            f"- Draft IRA deduction currently applied on Form 1040 line 10: {money(ira_deduction)}"
            if ira_deduction
            else "- No IRA deduction is applied yet. Confirm traditional, Roth, rollover, and nondeductible treatment before adding one."
        ),
    ]
