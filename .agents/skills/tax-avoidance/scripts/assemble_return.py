...
def build_dossier(normalized: dict[str, Any], line_items: list[dict[str, Any]]) -> str:
    ...
    social_security_review_lines = [
        (
            f"- SSA-1099 benefits spotted: {money(social_security)}"
            if social_security
            else "- No SSA-1099 benefits were detected."
        ),
        (
            f"- Taxable Social Security currently applied on Form 1040 line 6b: {money(taxable_social_security)}"
            if taxable_social_security
            else "- No taxable Social Security amount is applied yet. Review the SSA-1099 worksheet result before adding one."
        ),
    ]
