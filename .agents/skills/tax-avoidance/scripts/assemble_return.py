...
def build_dossier(normalized: dict[str, Any], line_items: list[dict[str, Any]]) -> str:
    ...
    ira_review_lines = [
        (
            f"- Traditional IRA contributions reported from Form 5498 documents: {money(ira_contributions_reported)}"
            if ira_contributions_reported
            else "- No Form 5498 IRA contribution amount is currently surfaced."
        ),
        (
            f"- Draft IRA deduction currently applied in the return package: {money(ira_deduction)}"
            if ira_deduction
            else "- No IRA deduction is applied yet. Confirm eligibility and the deductible amount before adding one."
        ),
    ]
