...
def build_dossier(normalized: dict[str, Any], line_items: list[dict[str, Any]]) -> str:
    ...
    ira_review_lines = [
        (
            f"- Reported IRA contributions spotted from Form 5498 documents: {money(reported_ira_contributions)}"
            if reported_ira_contributions
            else "- No Form 5498 IRA contribution amount is extracted yet."
        ),
        (
            f"- Draft IRA deduction currently applied on Form 1040 line 10: {money(ira_deduction)}"
            if ira_deduction
            else "- No IRA deduction is applied yet. Review Form 5498 contributions before adding one."
        ),
    ]
