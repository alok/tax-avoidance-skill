...
def build_dossier(normalized: dict[str, Any], line_items: list[dict[str, Any]]) -> str:
    ...
    ira_review_lines = [
        (
            f"- Traditional IRA contributions reported on Form 5498 documents: {money(ira_contributions_reported)}"
        ...
        (
            ...
            if ira_deduction
            else "- No IRA deduction is applied yet. Review filing status, workplace retirement coverage, and MAGI limits before adding one."
        ),
    ]
