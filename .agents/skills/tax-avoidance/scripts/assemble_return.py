...
def build_dossier(normalized: dict[str, Any], line_items: list[dict[str, Any]]) -> str:
    ...
    ira_review_lines = [
        (
            ...
            else "- No Form 5498 traditional IRA contributions were extracted."
        ),
        (
            f"- Draft IRA deduction currently applied on Form 1040 line 10: {money(ira_deduction)}"
            if ira_deduction
            else "- No IRA deduction is applied yet. Review Form 5498 and Publication 590-A rules before adding one."
        ),
    ]
