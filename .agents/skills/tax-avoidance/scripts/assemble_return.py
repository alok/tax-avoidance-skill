...
def build_dossier(normalized: dict[str, Any], line_items: list[dict[str, Any]]) -> str:
    ...
    ira_review_lines = [
        (
            f"- Traditional IRA contributions spotted from Form 5498 documents: {money(ira_contributions_reported)}"
            if ira_contributions_reported
            else "- No traditional IRA contribution evidence has been captured from Form 5498 documents."
        ),
        (
            f"- Draft IRA deduction currently applied on Form 1040 line 10: {money(ira_deduction)}"
            if ira_deduction
            else "- No IRA deduction is applied yet. Review deductibility before adding one to the draft return."
        ),
    ]
