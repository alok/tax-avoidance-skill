...
def build_dossier(normalized: dict[str, Any], line_items: list[dict[str, Any]]) -> str:
    ...
    social_security_review_lines = [
        f"- Gross Social Security benefits spotted from SSA-1099 documents: {money(fact_value(normalized, 'social_security_benefits'))}",
        (
            f"- Taxable Social Security currently applied on Form 1040 line 6b: {money(taxable_social_security)}"
            if taxable_social_security
            else "- No taxable Social Security amount is applied yet. Review the SSA-1099 and calculate the taxable portion before using line 6b."
        ),
    ]
