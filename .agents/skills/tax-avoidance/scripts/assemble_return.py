...
def build_dossier(normalized: dict[str, Any], line_items: list[dict[str, Any]]) -> str:
    ...
    ira_review_lines = [
        (
            f"- Form 5498 contribution evidence spotted: {money(ira_contribution_evidence)}"
            if ira_contribution_evidence
            else "- No Form 5498 IRA contribution evidence is present."
        ),
        (
            f"- Draft deductible IRA amount currently applied on Form 1040 line 10: {money(ira_deduction)}"
            if ira_deduction
