...
def build_dossier(normalized: dict[str, Any], line_items: list[dict[str, Any]]) -> str:
    ...
    social_security_review_lines = [
        ...
        (
            f"- Taxable Social Security benefits currently applied on Form 1040 line 6b: {money(taxable_social_security)}"
            if fact_sources(normalized, "taxable_social_security_benefits")
            else "- No taxable Social Security amount is applied yet. Review the SSA-1099 details before adding any taxable benefits to income."
        ),
    ]
