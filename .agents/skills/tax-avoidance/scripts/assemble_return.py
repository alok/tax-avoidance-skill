...
def build_dossier(normalized: dict[str, Any], line_items: list[dict[str, Any]]) -> str:
    ...
    social_security_review_lines = [
        f"- Gross SSA-1099 benefits spotted: {money(social_security) if social_security else '$0.00'}",
        (
            f"- Taxable Social Security currently applied on Form 1040 line 6b: {money(fact_value(normalized, 'taxable_social_security_benefits'))}"
            if fact_sources(normalized, "taxable_social_security_benefits")
            else "- Taxable Social Security is not applied yet. Review the SSA-1099 worksheet result before adding any amount to Form 1040 line 6b."
        ),
    ]
