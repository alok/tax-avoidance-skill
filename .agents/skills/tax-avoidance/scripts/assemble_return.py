...
def build_dossier(normalized: dict[str, Any], line_items: list[dict[str, Any]]) -> str:
    ...
    ira_deduction_applied = fact_value(normalized, "ira_contribution_deduction")
    retirement_review_lines = [
        (
            ...
            else "- No Form 5498 IRA contribution amount is captured yet."
        ),
        (
            f"- Draft IRA deduction currently applied on Form 1040 line 10: {money(ira_deduction_applied)}"
            if ira_deduction_applied
            else "- No IRA deduction is applied yet. Review deductibility before adding one."
        ),
    ]
