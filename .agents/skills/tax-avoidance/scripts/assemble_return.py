...
def build_dossier(normalized: dict[str, Any], line_items: list[dict[str, Any]]) -> str:
    ...
    retirement_review_lines = [
        f"- IRA contributions spotted from Form 5498 documents: {money(ira_contributions) if ira_contributions else '$0.00'}",
        (
            f"- Draft deductible IRA contribution currently applied in Form 1040 line 10 adjustments: {money(ira_deduction)}"
            if ira_deduction
            else "- No IRA deduction is applied yet. Review eligibility before adding one."
        ),
        "- Rule source: IRS Publication 590-A.",
    ]
