...
def build_line_items(normalized: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        ...
        },
        {
            ...
            if fact_sources(normalized, "taxable_social_security") or taxable_social_security > 0.0
            else None,
...
def build_dossier(normalized: dict[str, Any], line_items: list[dict[str, Any]]) -> str:
    ...
    social_security_review_lines = [
        f"- Total Social Security benefits spotted from SSA-1099 documents: {money(social_security) if social_security else '$0.00'}",
        (
            f"- Confirmed taxable Social Security currently applied on Form 1040 line 6b: {money(taxable_social_security)}"
            if fact_sources(normalized, "taxable_social_security") or taxable_social_security > 0.0
            else "- No taxable Social Security amount is applied yet. Review the SSA-1099 and benefits worksheet inputs before finalizing line 6b."
        ),
    ]
