...
def aggregate_numeric_fields(
    ...
    def first_nonzero_value(document: dict[str, Any]) -> tuple[str | None, float]:
    ...
    def source_rank(document: dict[str, Any]) -> tuple[int, int]:
        ...
        _, value = first_nonzero_value(document)
    ...
    sources: list[dict[str, Any]] = []
    for document in grouped_documents:
        field_name, value = first_nonzero_value(document)
        if field_name is None or value == 0.0:
            continue
        total += value
        sources.append(
            {
