...
- Produces the same artifact set every time:
  ...
  - `federal-lines.md`
  - `missing-items.md`
- Preserves public-safe dependent and household review scaffolding without collecting full SSNs in repo-facing artifacts.
...
This repository targets **simple federal individual returns** only: single or married-filing-jointly households with wage, contractor, and investment income plus common deductions and credits. It supports a simple Schedule C skeleton for contractor `1099-NEC` work when gross receipts are known and business expenses can be gathered. It also preserves dependent-review scaffolding such as relationship, birth year, months in home, support share, and TIN-status confirmation without storing full identification numbers. It still excludes rental income, K-1s, stock options, QSBS, trusts, estates, multistate returns, and international filings.
...
4. Build a document inventory, capture public-safe household/dependent context, and ask the minimum remaining interview questions.
