...
- Produces the same artifact set every time:
  ...
  - `federal-lines.md`
  - `missing-items.md`
- Preserves safe dependent and household intake context so child-credit and support-test follow-up can be reviewed later without storing SSNs.
...
This repository targets **simple federal individual returns** only: single or married-filing-jointly households with wage, contractor, and investment income plus common deductions and credits. It supports a simple Schedule C skeleton for contractor `1099-NEC` work when gross receipts are known and business expenses can be gathered. It also preserves dependent intake scaffolding for child/dependent-credit review, but it does not determine eligibility automatically or collect SSNs. It still excludes rental income, K-1s, stock options, QSBS, trusts, estates, multistate returns, and international filings.
...
4. Capture household and dependent context using safe public fields such as relationship, birth year, months at home, and support-test status.
