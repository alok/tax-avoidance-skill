...
- Preserves dependent and household review scaffolding without storing SSNs or silently deciding Child Tax Credit eligibility.
...
This repository targets **simple federal individual returns** only: single or married-filing-jointly households with wage, contractor, and investment income plus common deductions and credits. It supports safe household/dependent intake scaffolding, a simple Schedule C skeleton for contractor `1099-NEC` work when gross receipts are known and business expenses can be gathered, and `1098-T` tuition statements as education-credit review evidence. Dependent support is deliberately limited to review scaffolding: the artifacts preserve ages, relationship, residency/support context, and missing follow-up facts without collecting SSNs or silently computing final Child Tax Credit eligibility. It still excludes rental income, K-1s, stock options, QSBS, trusts, estates, multistate returns, and international filings.
...
4. Capture household/dependent review context with labels only, never SSNs, and ask the minimum remaining interview questions.
