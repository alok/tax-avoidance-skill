...
- Carries document-backed quarterly estimated tax payments into the draft federal lines and total-payments summary when `1040-ES` vouchers or IRS payment confirmations are available.
...
This repository targets **simple federal individual returns** only: single or married-filing-jointly households with wage, contractor, and investment income plus common deductions and credits. It supports a simple Schedule C skeleton for contractor `1099-NEC` work when gross receipts are known and business expenses can be gathered. It recognizes `1040-ES` and IRS estimated-payment confirmations for payment tracking, and it recognizes `1098-T` tuition statements as education-credit review evidence without silently computing the final credit. It still excludes rental income, K-1s, stock options, QSBS, trusts, estates, multistate returns, and international filings.
