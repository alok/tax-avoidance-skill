...
- Preserves `1098-T` tuition and scholarship figures in the artifact set so the remaining education-credit interview can stay targeted.

## Scope

This repository targets **simple federal individual returns** only: single or married-filing-jointly households with wage, contractor, and investment income plus common deductions and credits. It supports a simple Schedule C skeleton for contractor `1099-NEC` work when gross receipts are known and business expenses can be gathered. It also preserves `1098-T` tuition statement amounts as education-credit review scaffolding, without pretending the credit can be finalized from the form alone. It still excludes rental income, K-1s, stock options, QSBS, trusts, estates, multistate returns, and international filings.
