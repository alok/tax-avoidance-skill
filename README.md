...
- Produces the same artifact set every time:
  ...
  - `federal-lines.md`
  - `missing-items.md`
- Preserves household dependent context in a safe, review-oriented form so child-credit follow-up can happen without storing SSNs or ITINs in repo artifacts.
...
It now also supports safe **dependent intake scaffolding**: labels, relationship, birth year, months in home, student or disability flags, and care-expense follow-up. The flow deliberately avoids storing full SSNs, ITINs, or pretending to decide eligibility silently.
...
7. Preserve safe dependent-review context and surface any missing child-credit or care-credit follow-up.
