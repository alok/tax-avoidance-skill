...
- Produces the same artifact set every time:
  ...
  - `federal-lines.md`
  - `missing-items.md`
- Derives a structured interview queue from the normalized facts so the next questions are explicit, prioritized, and tied to answer keys.
...
6. Derive a structured interview queue with blocking vs non-blocking follow-ups and stable answer keys.
