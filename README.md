...
This repository targets **simple federal individual returns** only: single or married-filing-jointly households with wage, contractor, investment, and straightforward retirement-distribution income plus common deductions and credits. It supports a simple Schedule C skeleton for contractor `1099-NEC` work when gross receipts are known and business expenses can be gathered. It also supports `1099-R` intake when the taxable amount is explicitly available from the form. It still excludes rental income, K-1s, stock options, QSBS, trusts, estates, multistate returns, and international filings.
