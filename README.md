...
This repository targets **simple federal individual returns** only: single or married-filing-jointly households with wage, contractor, and investment income plus common deductions and credits. It supports a simple Schedule C skeleton for contractor `1099-NEC` work when gross receipts are known and business expenses can be gathered. It also supports simple `1099-B` summary inputs when net gain/loss is provided directly or can be derived from summary fields such as proceeds, basis, and wash-sale disallowance. It still excludes rental income, K-1s, stock options, QSBS, trusts, estates, multistate returns, and international filings.
