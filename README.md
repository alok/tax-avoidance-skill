...
- Preserves Social Security benefits intake safely by drafting Form 1040 lines `6a` and `6b`, while requiring an explicit taxable-benefit review before those amounts flow into total income.

## Scope

This repository targets **simple federal individual returns** only: single or married-filing-jointly households with wage, contractor, and investment income plus common deductions and credits. It supports a simple Schedule C skeleton for contractor `1099-NEC` work when gross receipts are known and business expenses can be gathered. Social Security intake is preserved, but the taxable-benefit amount still requires an explicit review input before it is rolled into totals. It still excludes rental income, K-1s, stock options, QSBS, trusts, estates, multistate returns, and international filings.
