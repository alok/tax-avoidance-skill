...
- Produces the same artifact set every time:
  ...
  - `federal-lines.md`
  - `missing-items.md`
- Makes the dossier easier to review by listing documented income, adjustments, deductions, credits, payments, and their IRS source links in a dedicated `Documented Tax Inputs` section.
...
7. Show documented tax inputs separately from values that were only preserved for review, so the user can see what still needs a standard-vs-itemized or credit decision.
