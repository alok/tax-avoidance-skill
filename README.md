...
- Tracks Form `5498` IRA contribution evidence and asks for deductibility review instead of silently treating the reported contribution as deductible.

## Scope

This repository targets **simple federal individual returns** only: single or married-filing-jointly households with wage, contractor, and investment income plus common deductions and credits. It supports a simple Schedule C skeleton for contractor `1099-NEC` work when gross receipts are known and business expenses can be gathered. It recognizes `1098-T` tuition statements as education-credit review evidence and `5498` IRA statements as contribution-review evidence, carrying those amounts into the artifact set without silently computing the final credit or IRA deduction. It still excludes rental income, K-1s, stock options, QSBS, trusts, estates, multistate returns, and international filings.
...
That should create the same four standard artifacts in `output/example-run/`. The sample input also includes a `5498` so you can see the IRA review section without using any personal tax data.
