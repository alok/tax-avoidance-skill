...
- Treats Form `5498` retirement contribution statements as review evidence, so IRA contributions stay visible in the artifacts without being silently treated as deductible.

## Scope

This repository targets **simple federal individual returns** only: single or married-filing-jointly households with wage, contractor, and investment income plus common deductions and credits. It supports a simple Schedule C skeleton for contractor `1099-NEC` work when gross receipts are known and business expenses can be gathered. It recognizes `1098-T` tuition statements and `5498` retirement contribution statements as review evidence, carrying those amounts into the artifact set without silently computing the final education credit or IRA deduction. It still excludes rental income, K-1s, stock options, QSBS, trusts, estates, multistate returns, and international filings.
