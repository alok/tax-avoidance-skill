...
- Produces the same artifact set every time:
  ...
  - `federal-lines.md`
  - `missing-items.md`
- Carries estimated-tax and extension-payment confirmations into the payment summary instead of forcing the user to retype them.
...
This repository targets **simple federal individual returns** only: single or married-filing-jointly households with wage, contractor, and investment income plus common deductions, credits, and payment confirmations. It supports a simple Schedule C skeleton for contractor `1099-NEC` work when gross receipts are known and business expenses can be gathered. It still excludes rental income, K-1s, stock options, QSBS, trusts, estates, multistate returns, and international filings.
