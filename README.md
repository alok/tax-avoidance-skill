...
- Produces the same artifact set every time:
  ...
  - `federal-lines.md`
  - `missing-items.md`
- Maps simple `SSA-1099` Social Security benefits into the draft federal line set so retirement-income households can keep a cleaner intake artifact.
...
This repository targets **simple federal individual returns** only: single or married-filing-jointly households with wage, contractor, investment, and straightforward `SSA-1099` Social Security income plus common deductions and credits. It supports a simple Schedule C skeleton for contractor `1099-NEC` work when gross receipts are known and business expenses can be gathered. It still excludes rental income, K-1s, stock options, QSBS, trusts, estates, multistate returns, and international filings.
