...
- Treats `SSA-1099` intake as review-first: gross benefits are captured, but the taxable amount is only applied when it is explicitly confirmed instead of being silently assumed.

## Scope

This repository targets **simple federal individual returns** only: single or married-filing-jointly households with wage, contractor, and investment income plus common deductions and credits. It supports a simple Schedule C skeleton for contractor `1099-NEC` work when gross receipts are known and business expenses can be gathered. It also recognizes `1098-T` tuition statements as education-credit review evidence and `SSA-1099` statements as Social Security review evidence, carrying those gross amounts into the artifact set without silently computing the final taxable amount. It still excludes rental income, K-1s, stock options, QSBS, trusts, estates, multistate returns, and international filings.
