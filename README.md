...
- Produces the same artifact set every time:
  ...
  - `federal-lines.md`
  - `missing-items.md`
- Preserves dependent intake context for child-credit workflows using interview-safe fields such as relationship, date of birth, months lived with taxpayer, and masked TIN last 4.
...
For dependent-related credits, the deterministic layer now expects a top-level `dependents` array with public-safe intake fields only, for example:
  ...
  "dependents": [
    {
      "name": "Child One",
      "relationship": "child",
      "date_of_birth": "2018-04-09",
      "tin_last4": "1234",
      "months_lived_with_taxpayer": 12,
      "qualifies_for_child_tax_credit": true
    }
  ]
}
```
