...
- Produces the same artifact set every time:
  ...
  - `federal-lines.md`
  - `missing-items.md`
- Adds targeted interview questions for common education, IRA, and HSA follow-up so the user knows exactly which next facts unlock the draft package.
...
This repository targets **simple federal individual returns** only: single or married-filing-jointly households with wage, contractor, and investment income plus common deductions and credits. It supports a simple Schedule C skeleton for contractor `1099-NEC` work when gross receipts are known and business expenses can be gathered. It also preserves common intake signals from forms such as `1098-T`, `5498`, `5498-SA`, and `1099-SA` so the workflow can ask better next questions even when those forms do not directly determine a final line by themselves. It still excludes rental income, K-1s, stock options, QSBS, trusts, estates, multistate returns, and international filings.
