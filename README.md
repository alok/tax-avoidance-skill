...
- Drafts `1099-R` pension and annuity lines conservatively by separating gross distributions from taxable amounts and flagging rollover review when the taxable amount is missing.

## Scope

This repository targets **simple federal individual returns** only: single or married-filing-jointly households with wage, contractor, investment, and straightforward retirement-distribution income plus common deductions and credits. It supports a simple Schedule C skeleton for contractor `1099-NEC` work when gross receipts are known and business expenses can be gathered. It still excludes rental income, K-1s, stock options, QSBS, trusts, estates, multistate returns, and international filings.
