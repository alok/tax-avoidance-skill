...
- Produces the same artifact set every time:
  ...
  - `federal-lines.md`
  - `missing-items.md`
- Computes a simple regular-method Schedule SE summary for supported `1099-NEC` contractor cases, including self-employment tax and the deductible half of that tax.
...
This repository targets **simple federal individual returns** only: single or married-filing-jointly households with wage, contractor, and investment income plus common deductions and credits. It supports a simple Schedule C skeleton for contractor `1099-NEC` work when gross receipts are known and business expenses can be gathered, plus a regular-method Schedule SE summary for self-employment tax. It still excludes rental income, K-1s, stock options, QSBS, trusts, estates, multistate returns, and international filings.
