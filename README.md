...
- Produces the same artifact set every time:
  ...
  - `federal-lines.md`
  - `missing-items.md`
- Preserves public-safe dependent intake scaffolding so household context, likely child-credit follow-up, and filing gaps are visible without storing full SSNs or dates of birth in the deterministic artifact layer.
...
This repository targets **simple federal individual returns** only: single or married-filing-jointly households with wage, contractor, and investment income plus common deductions and credits. It supports public-safe household/dependent scaffolding and a simple Schedule C skeleton for contractor `1099-NEC` work when gross receipts are known and business expenses can be gathered. It still excludes rental income, K-1s, stock options, QSBS, trusts, estates, multistate returns, and international filings.
...
If the user also needs household scaffolding, a stronger version is:

```text
Use $tax-avoidance to gather my 2025 tax docs, preserve public-safe dependent context, and tell me what child-credit facts are still missing before the return package is final.
```
...
3. Capture resident-state, work-state, and safe dependent-context scaffolding as early as possible.
