...
- Produces the same artifact set every time:
  ...
  - `federal-lines.md`
  - `missing-items.md`
- Carries quarterly estimated tax payments into the draft package when `1040-ES Payment` records are present, so Form 1040 payment totals are not understated.
...
This repository targets **simple federal individual returns** only: single or married-filing-jointly households with wage, contractor, and investment income plus common deductions and credits. It supports a simple Schedule C skeleton for contractor `1099-NEC` work when gross receipts are known and business expenses can be gathered, and it can preserve quarterly estimated tax payments for the federal payment lines. It still excludes rental income, K-1s, stock options, QSBS, trusts, estates, multistate returns, and international filings.
