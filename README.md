...
- Produces the same artifact set every time:
  ...
  - `federal-lines.md`
  - `missing-items.md`
- Preserves observed `1098-T` tuition amounts, scholarships, and `5498` IRA contributions as intake signals so the follow-up interview can ask for the right education-credit and IRA-deduction details.
...
This repository targets **simple federal individual returns** only: single or married-filing-jointly households with wage, contractor, and investment income plus common deductions and credits. It supports a simple Schedule C skeleton for contractor `1099-NEC` work when gross receipts are known and business expenses can be gathered. It also uses common intake forms like `1098-T` and `5498` to drive targeted follow-up questions for education credits and IRA deductions, without pretending those forms alone settle eligibility. It still excludes rental income, K-1s, stock options, QSBS, trusts, estates, multistate returns, and international filings.
