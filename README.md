...
- Captures `5498` IRA contribution evidence and turns it into a deductible-review prompt instead of silently applying it.

## Scope

This repository targets **simple federal individual returns** only: single or married-filing-jointly households with wage, contractor, and investment income plus common deductions and credits. It supports a simple Schedule C skeleton for contractor `1099-NEC` work when gross receipts are known and business expenses can be gathered. It also recognizes `1098-T` tuition statements as education-credit review evidence and `5498` IRA statements as retirement-contribution evidence, carrying those amounts into the artifact set without silently computing a final education credit or IRA deduction. It still excludes rental income, K-1s, stock options, QSBS, trusts, estates, multistate returns, and international filings.
...
For a safe retirement-review example:

```bash
uv run python .agents/skills/tax-avoidance/scripts/run_tax_flow.py \
  --input examples/ira-review-input.json \
  --out-dir output/ira-review-example
```

That example captures `5498` evidence in the dossier and missing-items output without silently applying an IRA deduction.
