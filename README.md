...
- Produces the same artifact set every time:
  ...
  - `federal-lines.md`
  - `missing-items.md`
- Queues the next intake questions in a deterministic order so the user can answer only what still blocks the draft package.
...
8. Queue the next intake questions with reasons and target answer keys so the interview can continue without a generic questionnaire.
