...
- Produces the same artifact set every time:
  ...
  - `federal-lines.md`
  - `missing-items.md`
- Preserves household and dependent context in a safe scaffold that keeps relationship, age, and only TIN last-four metadata instead of full SSNs.
...
Household support is intentionally conservative: the workflow can preserve dependent context and surface missing child-credit review inputs, but it does not decide eligibility from public heuristics alone and should never store full dependent SSNs.
...
There is also a family-oriented public example with dependent scaffolding:

```bash
uv run python .agents/skills/tax-avoidance/scripts/run_tax_flow.py \
  --input examples/family-w2-and-dependent-input.json \
