...
- Produces the same artifact set every time:
  ...
  - `federal-lines.md`
  - `missing-items.md`
- Carries a prioritized interview queue in the normalized return data and renders it in the public artifacts so the next highest-value question is explicit.
...
8. Clearly label the next interview questions, legal planning moves, missing items, unsupported complexity, state follow-up, and anything that needs professional review.
