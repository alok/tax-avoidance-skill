...
- Produces the same artifact set every time:
  ...
  - `federal-lines.md`
  - `missing-items.md`
- Preserves gross Social Security benefits from `SSA-1099` documents separately and requires an explicit taxable-benefits review before filling Form 1040 line 6b.
...
This repository targets **simple federal individual returns** only: single or married-filing-jointly households with wage, contractor, and investment income plus common deductions and credits. It supports a simple Schedule C skeleton for contractor `1099-NEC` work when gross receipts are known and business expenses can be gathered. It also preserves `SSA-1099` benefits for review, but does not guess the taxable portion without an explicit answer. It still excludes rental income, K-1s, stock options, QSBS, trusts, estates, multistate returns, and international filings.
