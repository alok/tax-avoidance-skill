...
- Produces the same artifact set every time:
  ...
  - `federal-lines.md`
  - `missing-items.md`
- Treats `SSA-1099` and `5498` intake conservatively: gross Social Security and reported IRA contributions are captured, while taxable benefits and deductible IRA amounts stay explicit review points unless already confirmed.
...
Some supported inputs still require explicit review rather than silent computation. For example, a Form `5498` contribution amount does not automatically equal the deductible IRA amount, and `SSA-1099` benefits should not be treated as fully taxable unless the taxable portion is already known.
