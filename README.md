...
- Produces the same artifact set every time:
  ...
  - `federal-lines.md`
  - `missing-items.md`
- Carries document-backed estimated tax and extension payments into the draft package instead of requiring them to be typed in manually.
...
This repository targets **simple federal individual returns** only: single or married-filing-jointly households with wage, contractor, and investment income plus common deductions and credits. It supports a simple Schedule C skeleton for contractor `1099-NEC` work when gross receipts are known and business expenses can be gathered. It recognizes `1098-T` tuition statements as education-credit review evidence, and it can also carry document-backed `1040-ES` estimated payments and `4868` extension payments into the artifact set without forcing manual re-entry. It still excludes rental income, K-1s, stock options, QSBS, trusts, estates, multistate returns, and international filings.
